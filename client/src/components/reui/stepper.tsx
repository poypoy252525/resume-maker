...
function Stepper({
  ...
  const registerTrigger = useCallback((node: HTMLButtonElement) => {
    ...
    return () => setTriggerNodes((prev) => prev.filter((n) => n !== node));
  }, []);
...
function StepperTrigger({
  ...
  // Find our index among triggers for navigation in the event handler to avoid ref access during render
  const handleKeyDown = (e: KeyboardEvent<HTMLButtonElement>) => {
    ...
    switch (e.key) {
      ...
      case "ArrowDown":
        e.preventDefault();
        if (focusNext) focusNext(myIdx);
        break;
      case "ArrowLeft":
      case "ArrowUp":
        e.preventDefault();
        if (focusPrev) focusPrev(myIdx);
        break;
      case "Home":
        e.preventDefault();
