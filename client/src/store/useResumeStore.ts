...
interface ResumeState {
  ...
  setFormData: (data: Partial<ResumeData> | ((prev: ResumeData) => ResumeData)) => void;
  ...
  // Helpers
  pollTask: <T>(taskId: string, interval?: number, maxRetries?: number) => Promise<T>;
    ...
    (set, get) => ({
      ...
      triggerAIAnalysis: async () => {
        ...
        try {
          ...
          set((state) => ({ formData: { ...state.formData, ai_feedback: feedback } }));
        } catch (err: unknown) {
      ...
      handleRecommendSkills: async () => {
        ...
        try {
          ...
          const result = await pollTask<{ recommended_skills: string[] }>(task_id);
          return result.recommended_skills;
        } catch (err: unknown) {
          set({ error: (err as Error).message || "Failed to recommend skills" });
          return null;
        } finally {
          set({ loading: false });
        }
